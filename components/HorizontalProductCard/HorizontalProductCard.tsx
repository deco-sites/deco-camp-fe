...
export default function HorizontalProductCard({ product, animateImage }: HorizontalProductCardProps) {
  ...
  const eventItem = mapProductToAnalyticsItem({ product, breadcrumbList: undefined, price, listPrice })

  return (
    <a href={url} aria-label={`Ver produto ${name}`} class="flex gap-2 sm:gap-4 md:gap-8 p-2 sm:p-4 md:p-6 rounded-xl bg-neutral-content">
      {!!image && (
      ...
      {!image && <div class="w-40 md:w-48 h-40 md:h-48 rounded bg-gray-300"></div>}
      ...
      <div class="flex flex-col md:flex-row gap-3 sm:gap-4 md:gap-8 flex-1 ">
        ...
        <div class="flex flex-col gap-1 md:gap-8 md:pl-8 md:border-l md:border-solid md:border-gray-300">
          ...
          <div class="flex flex-col gap-2 mt-auto max-w-48 md:max-w-none">
            {!!price && <AddToCartButtonVTEX eventParams={{ items: [eventItem] }} productID={id} seller={'1'} />}
