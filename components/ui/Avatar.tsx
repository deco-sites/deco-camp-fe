...
const colors: Record<string, string> = {
  ...
  active: 'text-base-content ring-1 ring-black',
...
function Avatar({ content, variant = 'default' }: Props) {
  return (
    <div class="avatar placeholder text-sm font-light h-6">
      <div
        class={`${colors[content] ?? colors[variant]} ${variants[variant]} checkbox`}
        role="checkbox"
        aria-checked={variant === 'active'}
        aria-label={`Seletor do filtro ${content}`}
      >
        <span class="uppercase text-xs">{colors[content] ? '' : content.substring(0, 2)}</span>
      </div>
    </div>
  )
}
