...
export default function BannnerGrid(props: Props) {
  const { title, itemsPerLine, borderRadius, banners = [] } = { ...DEFAULT_PROPS, ...props }

  return (
    <section class="container w-full px-4 md:px-0 mx-auto py-8 lg:py-10">
      {title && (
        <div class="py-6 md:py-0 md:pb-[40px] flex items-center relative justify-center">
          <h2 class="bg-white text-lg leading-5 font-semibold uppercase text-center z-10 px-4">{title}</h2>

          <div class="bg-[#e5e5ea] h-[1px] w-full absolute"></div>
        </div>
      )}
      <div class={`grid gap-4 md:gap-6 ${MOBILE_COLUMNS[itemsPerLine?.mobile ?? 2]} ${DESKTOP_COLUMNS[itemsPerLine?.desktop ?? 4]}`}>
        {banners.map(({ href, srcMobile, srcDesktop, alt }) => (
          <a
            href={href}
            class={`overflow-hidden ${RADIUS_MOBILE[borderRadius.mobile ?? 'none']} ${RADIUS_DESKTOP[borderRadius.desktop ?? 'none']} `}
          >
            <Picture>
              <Source media="(max-width: 767px)" src={srcMobile} width={100} height={100} />
              <Source media="(min-width: 768px)" src={srcDesktop ? srcDesktop : srcMobile} width={250} height={250} />
