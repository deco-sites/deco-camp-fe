...
export default function HorizontalProductCard({ products, animateImage }: Props) {
  if (!products?.length) return ErrorFallback({ error: new Error('Sem produtos para renderizar') })
  ...
  return (
    <div class="w-full max-w-5xl px-4 mx-auto py-8 lg:py-10 flex flex-col gap-8 lg:gap-10">
      {products.map((product) => {
        ...
        if (!name) return ErrorFallback({ error: new Error('Produto sem nome') })
